...
KN3_predict_withstopwords<-function(gram_1,gram_2,gram_3,inputstring,word_pred_list=data.frame(data.frame(pred_word=character(0),probability=numeric(0)))){
  cleanedinputstring<-clean_string(inputstring)
  inputstring_vec<-get_last_words(cleanedinputstring,2)
  if (length(inputstring_vec)<2){
    KN2_predict_no_KN3(gram_1,gram_2,inputstring_vec[1])
  }
  else{
  ...
  if (length(index)!=0){
    ...
    N3<-length(index)
    KN3_prob<-gram_3$Fraction[index]+gram_3$D[index]*(N3/sum(gram_3$Freq))*KN2_predict(gram_2,word_pred_list$pred_word,inputstring_vec[2])
    word_pred_list$KN3_prob<-KN3_prob
    word_pred_list<-word_pred_list[order(-KN3_prob),]
    return(word_pred_list)
    
    } else {
    KN2_predict_no_KN3(gram_1,gram_2,inputstring_vec[2])
  }
  }
}
...
KN2_predict<-function(gram_2,pred_string_vec,last_input_word){
  KN1_prob<-numeric(0)
  KN2_prob<-numeric(0)
...
KN2_predict_no_KN3<-function(gram_1,gram_2,last_input_word,word_pred_list=data.frame(data.frame(pred_word=character(0),probability=numeric(0)))){
  index<-which(gram_2$Word.1==last_input_word)
if (length(index)!=0){
  ...
  word_pred_list<-data.frame(gram_1$Phrase,gram_1$Prediction)
  names(word_pred_list)<-c("pred_word","probability")
  return(word_pred_list)
}
}
...
KN3_predict_nostopwords<-function(gram_1,gram_2,gram_3,inputstring,word_pred_list=data.frame(data.frame(pred_word=character(0),probability=numeric(0)))){
...
KN3_predict_nostopwords_stemmed<-function(gram_1,gram_2,gram_3,inputstring,word_pred_list=data.frame(data.frame(pred_word=character(0),probability=numeric(0)))){
